...
interface UserApi {
   ...
   suspend fun getUserForProfile(@Header("Authorization") token: String): Response<ApiResponse<UserForProfileModel>>

   @DELETE(Endpoints.DELETE_PROFILE)
   suspend fun delete(@Header("Authorization") token: String): Response<ApiResponse<Nothing>>
}